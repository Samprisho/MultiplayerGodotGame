...
class PlayerState:
	...
	func _init(pos: Vector3 = Vector3.ZERO, vel: Vector3 = Vector3.ZERO, time: int = 0, seq: int = 0):
		position = pos
		velocity = vel
		timestamp = time
		sequence_number = seq
...
class PlayerInput:
	...
	func _init(input: Vector2 = Vector2.ZERO, time: int = 0, seq: int = 0):
		motion = input
		timestamp = time
		sequence_number = seq
...
var position_tolerance: float = 0.7
var velocity_tolerance: float = 1.8

func _enter_tree():
	set_multiplayer_authority(name.to_int())

func _ready():
	current_predicted_state = PlayerState.new(global_position, velocity, Network.lobbyTime, 0)
...
func _physics_process(delta: float) -> void:
	...
	var player_input = PlayerInput.new(motion, Network.lobbyTime, sequence_counter)
...
func simulate_movement_with_physics(input: PlayerInput, delta: float) -> PlayerState:
	...
	var calculatedVelocity = calculate_movement(delta, input.motion)
...
func simulate_movement_for_replay(state: PlayerState, input: PlayerInput, delta: float) -> PlayerState:
...
func send_input_to_server(input: PlayerInput):
	...
	var motion_x = int(input.motion.x * 127) if input.motion.x >= 0 else (256 + int(input.motion.x * 127))
	var motion_y = int(input.motion.y * 127) if input.motion.y >= 0 else (256 + int(input.motion.y * 127))
	packet_data.append(motion_x)
	packet_data.append(motion_y)
	
	Network.udpClient.put_packet(packet_data)

func receive_server_correction(server_position: Vector3, server_velocity: Vector3, server_sequence: int):
	...
	print("Applying server correction. Pos error: %.2f, Vel error: %.2f" % [position_error, velocity_error])
	
	# Apply server correction by creating corrected state
	var corrected_state = PlayerState.new(server_position, server_velocity, old_state.timestamp, server_sequence)
	...
	for i in range(input_index + 1, input_buffer.size()):
		var input_to_replay = input_buffer[i]
		
		# Apply movement using the same physics
		var calculatedVelocity = calculate_movement(assumed_delta, input_to_replay.motion)
		velocity = calculatedVelocity
		move_and_slide()
		
		# Store the replayed state
		var replayed_state = PlayerState.new(global_position, velocity, input_to_replay.timestamp, input_to_replay.sequence_number)
...
func calculate_movement(delta: float, motion: Vector2) -> Vector3:
